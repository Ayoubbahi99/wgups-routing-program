...
def lookup_single_package(hash_table):
    ...
    if time_str:
        ...
        query_time = datetime.timedelta(hours=23, minutes=59)

    pkg.update_status(query_time)
    display_street, display_zip = pkg.get_display_address(query_time)

    print(f"\n  {'═' * 50}")
    print(f"  Package {pkg.ID}")
    print(f"  {'═' * 50}")
    print(f"  Address  : {display_street}")
    print(f"  City     : {pkg.city}")
    print(f"  State    : {pkg.state}")
    print(f"  Zip      : {display_zip}")
...
def main():
    ...
    truck1 = Truck(18, "4001 South 700 East", datetime.timedelta(hours=8),
                   [1, 29, 7, 30, 8, 34, 40, 14, 15, 16, 19, 20, 13, 37])
